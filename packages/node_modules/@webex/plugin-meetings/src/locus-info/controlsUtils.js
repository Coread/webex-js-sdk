...
ControlsUtils.parseStage = (controls) => {
  let stage;

  const {videoLayout} = controls;

  if (videoLayout) {
    const {stageParameters} = videoLayout;
...
ControlsUtils.parse = (controls) => {
  ...
  if (controls) {
    parsedControls.stage = ControlsUtils.parseStage(controls);
  }
