...
const Board = WebexPlugin.extend({
  ...
  createChannel(conversation, channel) {
    const body = this._prepareChannel(conversation, channel);
    ...
    return this.webex.request({
      ...
      console.log('CREATE ERROR', error, body.kmsMessage, body.defaultEncryptionKeyUrl);

      this.decryptSingleContent(body.defaultEncryptionKeyUrl, body.kmsMessage).then((result) => { console.log(result); });
      throw error;
    })
      .then((res) => res.body);
  },
  ...
  _prepareChannel(conversation, channel) {
    const start = {
      aclUrlLink: conversation.aclUrl || 'https://acl-a.wbx2.com/acl/api/v1/acls/bd76c700-02a9-11ed-962f-2133f3a63785',
      kmsMessage: {
        method: 'create',
        uri: '/resources',
        userIds: conversation.kmsResourceObjectUrl ? [conversation.kmsResourceObjectUrl] : [],
        keyUris: []
      }
    };

    const thing = Object.assign(start, channel);
